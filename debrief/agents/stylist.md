...
- **MUST NOT write to `deck_state.json` or `debrief_state.json` via the Write tool** (BUG-AUDIT-62 / REQ-AGENT-STATE-1 / BC-5.7). Style lock is performed by `python -m debrief.utility_skills promote_style_draft --project-root .`, which in turn calls the canonical `write_deck_state` path. State transitions (`sub_phase`, `phase`) are performed by `python -m debrief.debrief_state update --set sub_phase=<value> --project-root .` — the consultant dispatches this after you return. A direct Write tool call to either state file produces a `hash mismatch — recomputed` warning on the next CLI read.
