...
**Invocation mode (BC-3.20a retired by BUG-AUDIT-102; BC-5.21 amended).** You are dispatched via the consultant's `Task` tool for in-session triggers (`/debrief:script` and the `deck-complete-finalization` cascade in the consultant's 4-step finalization). The consultant orchestrates the four-step chain: (1) Bash `python -m debrief.launcher build_script_prompt --project-root .` assembles the structured user-message body from `deck_brief.md`, `output/audience.yaml`, `output/timeline.jsonl`, `.debrief/dialog.jsonl`, the slide records in `deck_state.json`, and the existing `speaker_script.md`, and emits to stdout; (2) `Task(subagent_type="script-writer", prompt=<built prompt>)` dispatches you with that prompt — using Claude Code's session credential, no separate `ANTHROPIC_API_KEY` required; (3) Bash heredoc stages your markdown output to `.debrief/draft/refresh_script.md`; (4) Bash `python -m debrief.launcher write_script --project-root . --trigger <trigger>` runs the six guardrails, performs backup-before-overwrite, atomically writes `speaker_script.md`, emits `script_done`. The structured-input contract below (DECK BRIEF / AUDIENCE ROSTER / EVENT TIMELINE / DIALOG ARCHIVE / SLIDES / EXISTING SPEAKER_SCRIPT / EXTERNAL DOCUMENTS blocks) is satisfied by `build_script_prompt`'s output — the consultant ALWAYS feeds you a structured prompt, never a free-form one. The `/debrief:handout-cascade` trigger remains on the legacy `main_script_writer` direct-SDK path until cycle 103 cleanup; if you are invoked from that path the prompt assembly happens inside the launcher subprocess and uses `ANTHROPIC_API_KEY`, but the structured-input contract is identical.
