...
- **REQ-SCRIPT-2:** *(Superseded by BUG-AUDIT-84 / REQ-SCRIPT-WRITER-1.)* The original requirement specified versioned script output at `output/<presentation_folder>/script_v{NNN}.md`. As of BUG-AUDIT-84, the script collapses to a single canonical artifact at `<project_root>/speaker_script.md`. Versioned outputs are retired; the prior `speaker_script.md` is backed up to `.debrief/script_backups/speaker_script.<UTC ISO 8601>.md` before each overwrite per REQ-SCRIPT-WRITER-3. See `spec/script_writer_rfc.md` for the architectural rationale.
...
- **REQ-HAND-3:** *(BUG-AUDIT-68 — rewritten; further amended by BUG-AUDIT-84 — see BC-11.15b for the collapsed single-source contract.)* The explanatory text for each slide is assembled from a deterministic three-level precedence, in order. The handout module applies the first source that yields non-empty text:
...
- **REQ-HAND-6:** *(BUG-AUDIT-21 — rewritten; further amended by BUG-AUDIT-84 — handout precondition adds `speaker_script.md` existence requirement; see BC-11.16 amendment.)* The handout skill is **independent of `/debrief:export`** and runs on any project with at least one approved non-backup slide. Output goes to `output/handouts/handout_v{NNN}.pdf`, where `NNN` is filesystem-derived: the handout module scans `output/handouts/` for existing `handout_v*.pdf` files and picks `(max + 1)` (or `1` if none). The `output/handouts/` directory is created on first invocation via `mkdir(parents=True, exist_ok=True)`. Handout does NOT consult `deck_state.presentations` and does NOT mutate `deck_state.json`. Preconditions (validated at entry, before any Playwright import or browser launch, in the order defined by BC-11.16): (1) `deck_state.json` exists and is readable; (2) at least one slide has `status == "approved"` and `backup != True`. On any precondition failure, the module prints a descriptive message identifying the missing prerequisite and exits with code 2. Rationale: a user may want to produce a handout without first generating a projected-deck PDF — the handout is a different artifact with different use cases (distribution, pre-reading, note-taking reference). See BC-11.15 (handout.css), BC-11.16 (fail-fast preconditions), BC-11.17 (decoupled output path), and BUG-AUDIT-21.
...
### BUG-AUDIT-84: Script writer architecture RFC adopted — agent-owned canonical `speaker_script.md`, six guardrails, auto-finalization, handout simplification

**Status:** Sub-cycle A — spec + blueprint contracts only. Sub-cycle B (script-writer agent + CLI) and Sub-cycle C (handout simplification + auto-finalization at deck-complete) follow per `spec/script_writer_rfc.md` §13.

**Problem (recap of the architectural conversation that produced the RFC).** The current `/debrief:script` is a deterministic template renderer. It produces correct structure but flat prose, ignores most of the memory architecture (audience roster, timeline, dialog), and yields versioned `script_v{NNN}.md` outputs that create ambiguity about which version is "the script." Independently, `/debrief:handout` falls back to `SlideRecord.content_summary` when no `speaker_script.md` is present (BC-11.15a / BUG-AUDIT-68), meaning a handout can be built from never-spoken text — the worst possible leave-behind.

**Architecture (full design in `spec/script_writer_rfc.md`).** Mirror the rewriter pattern (BUG-AUDIT-80): a new `agents/script-writer.md` agent-card carries the system prompt; a wrapping CLI (`script_writer` subcommand) reads inputs, calls the Anthropic API directly via the hybrid invocation pattern, validates the output against six guardrails, atomically writes `<project_root>/speaker_script.md` after backing up the prior version to `.debrief/script_backups/speaker_script.<UTC ISO 8601>.md`. Versioned `script_v{NNN}.md` outputs are retired.

The six guardrails: (1) source traceability (no invention of facts, names, numbers, or citations beyond brief/dialog/timeline/slides/roster); (2) no new positions (script doesn't editorialize beyond what the user surfaced); (3) per-slide structure fixed (one section per slide, canonical layout — prose inside is free); (4) length budget per slide (target = duration / slide_count, slack ≤ 50% before flagging); (5) roster-aware mentions (a roster member named in the script must have notes that plausibly match the slide's topic); (6) co-writer mode (when `speaker_script.md` already exists, the agent prefers the user's verbatim phrasing for unchanged content and writes new sections in a voice consistent with the user's existing prose; the validator flags voice drift via Jaccard bigram similarity below 0.5 on slides whose source data is unchanged).

`/debrief:handout` becomes a pure packager: requires `speaker_script.md` to exist (auto-cascade if missing — consultant runs `/debrief:script` first); collapses BUG-AUDIT-68's three-level notes precedence to a single source. The placeholder rule (silent empty cells forbidden) is preserved.

The consultant's `## Export Transition` is amended for an auto-finalization milestone: at deck-complete, the consultant proposes the four-step sequence `/debrief:refresh-brief` → `/debrief:script` → `/debrief:export` → `/debrief:handout`. Individual slash commands remain manually invocable. No new `/debrief:finalize` command — orchestration stays in the consultant.

External-document context (paper PDFs for journal_club, thesis PDFs for thesis_discussion, proposal PDFs for grant_panel) is **deferred** to BUG-AUDIT-85. The script-writer's input schema reserves a forward-compatible `external_documents` slot — always present in v1, always empty — with the per-entry shape (`slide_slug`, `paper_path`, `figure_num`, `caption`, `results_paragraph`, `figure_image_path`) documented now so v2 lands as population, not schema design.

**Detection method (forward-looking).** Once Sub-cycles B and C ship, every deck completion auto-generates a presenter-ready `speaker_script.md` whose prose draws on the audience roster, timeline events, and dialog archive. The script writer becomes a real co-writer — and the handout is a faithful packaging of slides plus the spoken script.

**Fix summary (Sub-cycle A — this BUG-AUDIT entry).** Spec + blueprint contracts only. The four normative requirements below + the corresponding behavioral contracts in `blueprint_contracts.md` (BC-3.20, BC-5.21, BC-11.15b, BC-11.20 added; BC-11.6, BC-11.15a, BC-11.16, BC-11.19 amended). No code lands in this sub-cycle. Sub-cycle B implements the agent + CLI; Sub-cycle C handles handout simplification + auto-finalization.

**Normative requirements:**

- **REQ-SCRIPT-WRITER-1:** A new script-writer agent at `agents/script-writer.md` MUST be the SOLE writer of `<project_root>/speaker_script.md`. The agent's frontmatter declares `model: claude-sonnet-4-6` (no user-configurable override), `maxTurns: 1`, `tools: Read`. The agent is invoked via the wrapping CLI (BC-3.20) using the hybrid pattern from BC-5.19 — the CLI extracts the agent-card system prompt, calls the Anthropic API directly, validates the output, and writes the file atomically. No other code path MAY write `speaker_script.md`. Versioned `output/<presentation_folder>/script_v{NNN}.md` outputs are retired; legacy projects' existing versioned files are not migrated, but new generations stop producing them. This requirement supersedes REQ-SCRIPT-2 (BUG-AUDIT-25's filesystem-derived versioning rule).

- **REQ-SCRIPT-WRITER-2:** The wrapping CLI MUST validate the script-writer agent's output against six guardrails before atomic write. Validation failures cause the CLI to log to `.debrief/script_errors.jsonl` (schema mirroring `.debrief/rewrite_errors.jsonl`: `timestamp`, `trigger`, `error_class`, `error_message`) and exit 0 — the prior `speaker_script.md` is retained. Warnings (voice-drift, length-budget overrun) are logged with `error_class: warning_<rule>` but do NOT block the write. The six guardrails: (1) **source traceability** — code-side validators check names against `output/audience.yaml`, numerics against brief/slides/dialog, citations against `paper_attached` events; (2) **no new positions** — prompt-only rule for v1; (3) **per-slide structure** — section count equals approved-non-backup-slide count plus an optional `## Backup Slides` section; each section has the four required subsections; (4) **length budget** — slides exceeding `(total_duration / slide_count) × 1.5` are flagged; (5) **roster-aware mentions** — name mentions require ≥1 keyword match between the roster entry's `notes` and the slide's `content_summary` / `visual_approach`; (6) **co-writer mode** — when prior `speaker_script.md` exists, sections of slides with unchanged source data MUST preserve user phrasing verbatim; new sections MUST adopt the user's voice; the validator flags voice drift via Jaccard bigram similarity below 0.5 on unchanged-source slides.

- **REQ-SCRIPT-WRITER-3:** Before each script generation, if `<project_root>/speaker_script.md` already exists, the wrapping CLI MUST copy it to `.debrief/script_backups/speaker_script.<UTC ISO 8601>.md` (the timestamp uses `Z` suffix and underscores in place of separators if needed for filesystem safety). Backups are retained indefinitely; the user MAY delete `.debrief/script_backups/` manually. Backup-write failures are logged but do NOT block the new generation (the source data is the canonical record; backups are a recovery convenience). The atomic write to `speaker_script.md` itself uses the `.tmp` + fsync + rename pattern; on rename failure, the prior `speaker_script.md` remains intact and the failure is logged.
