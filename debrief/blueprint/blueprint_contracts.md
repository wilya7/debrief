...
**BC-1.18 Anthropic SDK is a declared dependency (BUG-AUDIT-93, RETIRED by BUG-AUDIT-103).** *(Pre-fix history: this BC mandated `anthropic>=0.40` in both `pyproject.toml` `[project.dependencies]` and `environment.yml`'s pip section, with the smoke test importing it. The mandate was correct at BUG-AUDIT-93 time because the launcher's hybrid invocation pattern hit the SDK on every canonical rewriter and script-writer trigger. Cycles 101 and 102 moved those triggers to Task-dispatch via the consultant; the SDK is now only used by legacy direct-CLI paths and the BUG-AUDIT-98 auth-error helpers — none of them on a canonical user flow. Cycle 103 demotes the dep accordingly.)* This BC is RETIRED. See BC-1.18a for the post-cycle-103 contract: `anthropic` is OPTIONAL (under `[project.optional-dependencies]` with extras key `sdk_fallback`) and is NOT in the smoke test. The `tests/regressions/test_bug_audit_93_anthropic_dependency.py` regression test is INVERTED in cycle 103 — it now asserts the OPPOSITE of what BC-1.18 mandated.

**BC-1.18a Anthropic SDK is an optional dependency (BUG-AUDIT-103).** `anthropic>=0.40` MUST appear in `pyproject.toml` under `[project.optional-dependencies]` with extras key `sdk_fallback` (NOT under `[project.dependencies]`). It MUST NOT appear in `environment.yml`'s pip section. The `bin/debrief` step 5.5 smoke test MUST NOT import `anthropic`; the smoke-test imports list is `playwright, pptx, fitz, json_repair` (back to its pre-BUG-AUDIT-93 shape). Users who explicitly invoke the legacy direct-SDK paths (`python -m debrief.launcher script_writer` direct, the `--trigger /debrief:handout-cascade` choice, or the `main_rewrite_brief` synthesis branch) install the SDK with `pip install '.[sdk_fallback]'` after the normal install. Without that opt-in, the legacy paths fail with the BUG-AUDIT-93 missing-module stderr line, which is now the correct UX: those paths are documented dead-code-with-tests, not canonical flows. A regression test in `tests/regressions/test_bug_audit_103_anthropic_optional.py` enforces: (a) `anthropic` is NOT in `[project.dependencies]`; (b) `anthropic` IS in `[project.optional-dependencies] sdk_fallback`; (c) `environment.yml`'s pip section does NOT contain `anthropic`; (d) the smoke test in `bin/debrief` does NOT import `anthropic`. The original `test_bug_audit_93_anthropic_dependency.py` is updated in cycle 103 to assert the inverse of its original four invariants — same test name, opposite contract, mapping to the BC-1.18 retirement.
...
- On failure: appends a JSON entry to `.debrief/rewrite_errors.jsonl` (schema: `timestamp`, `trigger` ∈ `"PreCompact" | "/debrief:quit" | "/debrief:refresh-brief" | "session_start_recovery"`, `error_class`, `error_message`, optional `transcript_path`) and **exits 0**. The hook MUST NOT block compaction on rewrite failure (REQ-MEMORY-REWRITE-4). When the failure is specifically `ModuleNotFoundError` for the `anthropic` SDK (BUG-AUDIT-93), the launcher MUST additionally emit a single actionable line to stderr — `"rewrite_brief: anthropic SDK not installed in this env; install with 'pip install anthropic>=0.40' and retry. If the env was created by bin/debrief, run 'debrief --rebuild-env'."` — so the user knows the brief did not synthesize. **BUG-AUDIT-98 amendment:** when the failure is an Anthropic auth error — either `TypeError` whose stringified message contains `"could not resolve authentication"` (case-insensitive; the SDK constructor failure when no credential is in the environment) OR an exception whose class name is `"AuthenticationError"` (the SDK's `anthropic.AuthenticationError` raised on HTTP 401) — the launcher MUST also emit a single actionable line to stderr: `"rewrite_brief: anthropic API authentication failed; set ANTHROPIC_API_KEY in the environment and retry. Details logged to .debrief/rewrite_errors.jsonl."`. Exit code remains 0 unconditionally for the rewriter; PreCompact must never block. All other exception types (network errors, rate-limit errors, validation failures) keep the silent-exit-0 behavior — only the two well-known categories (missing module, auth) get user-visible stderr. **BUG-AUDIT-101 amendment:** the synthesis path is RETIRED from this subcommand for in-session triggers — `/debrief:refresh-brief`, `/debrief:quit` flush, and the new `session_start_recovery` trigger are routed via the consultant's `Task`-dispatch + `build_rewrite_prompt` + `write_brief` chain (BC-3.18a, BC-3.18b). The PreCompact trigger uses the capture-only contract in BC-3.18c. The two anthropic-error stderr branches above remain wired to handle any residual direct-SDK failure modes during the transition, but on a fully-converged code path they are unreachable.

**BC-3.18a `build_rewrite_prompt` subcommand (BUG-AUDIT-101).** `src/unit_3/launcher.py` MUST provide a `build_rewrite_prompt` subcommand wired into `main_new()`'s dispatch table. The subcommand:

- Parses `--project-root` (Path, default `Path.cwd()`).
- Reads project state via existing helpers: `read_dialog_archive(project_root)`, `read_event_timeline(project_root)`, `_read_rewrite_metadata(project_root)`, and the prior `deck_brief.md` (when present, used only as bootstrap input on the first rewrite per REQ-MEMORY-REWRITE-3).
- Calls the existing `build_rewrite_inputs(dialog, timeline, prior_brief)` helper to assemble the structured user message.
- Writes the assembled prompt to **stdout** (NOT stderr — the consultant captures this output via Bash and feeds it as the `prompt` argument to `Task(subagent_type="rewriter", ...)`). No headers, no JSON wrapping — the raw prompt text.
- Exit codes: 0 on success, 1 on irrecoverable read failure (e.g., project_root does not exist).
- Read-only — does not modify any file.
- The subcommand MUST appear in the usage-line string printed by the catch-all `else` branch in `main_new()`.

**BC-3.18b `write_brief` subcommand (BUG-AUDIT-101).** `src/unit_3/launcher.py` MUST provide a `write_brief` subcommand wired into `main_new()`'s dispatch table. The subcommand:

- Parses `--project-root` (Path, default `Path.cwd()`), `--trigger` (one of `"/debrief:refresh-brief"`, `"/debrief:quit"`, `"session_start_recovery"`, default `"/debrief:refresh-brief"`), and `--draft-path` (Path, default `<project_root>/.debrief/draft/refresh_brief.md`).
- Reads the agent's markdown output from `--draft-path`. If the path does not exist or is empty, logs an error to `.debrief/rewrite_errors.jsonl` (existing schema, with the new trigger value enumerated) and exits 0 (REQ-MEMORY-REWRITE-4 unchanged).
- Runs the SAME validators as the legacy `main_rewrite_brief`: `validate_brief_structure(response_text)` (canonical sections per REQ-CONSULT-DECK-BRIEF-1) and roster YAML extraction with the existing per-entry `name`/`role` checks. Validation failure logs and exits 0.
- On validation success: atomic dual-write via the existing `.tmp` + fsync + rename pattern. The write order MUST match `main_rewrite_brief`'s pattern: write `deck_brief.md.tmp`, write `output/audience.yaml.tmp`, atomically rename both, then update `.debrief/rewrite_metadata.json` (existing schema: `last_rewrite_timestamp`, `last_archived_turn`, `agent_version`, `model`, `bootstrap_complete`). On any rename failure, roll back per BC-3.18 (the dual-rename invariant is unchanged).
- On success: removes the draft file at `--draft-path` (cleanup). If the project had a `.debrief/.brief_stale` sentinel (set by PreCompact per BC-3.18c), the subcommand MUST also remove the sentinel — successful synthesis clears the stale-flag.
- Always exits 0 — REQ-MEMORY-REWRITE-4 unchanged.
- Read-then-write only on `deck_brief.md`, `output/audience.yaml`, `.debrief/rewrite_metadata.json`, `.debrief/draft/refresh_brief.md` (deletion), `.debrief/.brief_stale` (deletion). NEVER touches dialog archive, timeline, or any file outside this set.

**BC-3.18c `rewrite_brief --trigger PreCompact` capture-only contract (BUG-AUDIT-101).** When `main_rewrite_brief` is invoked with `--trigger PreCompact`, the subcommand MUST follow the capture-only path:

1. Read the stdin envelope (existing behavior). Parse the JSON `transcript_path` field. If present, call `append_dialog_turns_from_transcript(project_root, Path(transcript_path_str))` to capture new dialog turns into `.debrief/dialog.jsonl`. Stdin-handling defensiveness from the pre-existing implementation is preserved (skip silently when `sys.stdin.isatty()` or `read()` raises).
2. Compute the post-capture watermark: `last_archived_turn` from `.debrief/rewrite_metadata.json` (or 0 if metadata absent), updated to reflect the new turns just appended.
3. Write the sentinel `<project_root>/.debrief/.brief_stale` containing JSON: `{"timestamp": "<ISO 8601 UTC>", "last_archived_turn": <int>, "compaction_event_id": <string|null>, "trigger": "PreCompact"}`. Atomic write via `.tmp` + `os.rename`. The `compaction_event_id` is sourced from the stdin envelope when present (Claude Code may emit a unique id per compaction); else `null`.
4. **Make NO model call. Make NO `anthropic.Anthropic()` instantiation. Skip everything from the legacy `main_rewrite_brief` step 1 onward (agent card resolution, prompt assembly, API call, validation, atomic write).** The PreCompact path is now data-preservation-only.
5. Exit 0 unconditionally. Per BC-3.18, the schema for any failure log entry includes `trigger: "PreCompact"`; capture-only failures (e.g., transcript file missing) still log and exit 0.

The PreCompact path's runtime budget is dominated by step 1 (transcript file read + jsonl append). Step 3 is O(1). The pre-fix budget was step 1 + agent card read + Anthropic API roundtrip (multi-second); the post-fix budget is step 1 + sentinel write. The hook's 60-second timeout (`hooks.json`) becomes very loose for this minimal workload.

**BC-5.19 amendment for Task-dispatchability (BUG-AUDIT-101).** `agents/rewriter.md` MUST be Task-dispatchable. The agent card body (system prompt) is unchanged. The agent's `tools: Read` declaration is unchanged. The card MUST NOT include any prose claiming "this agent is not invoked via the Task tool" — that claim is RETIRED. The consultant invokes the agent via `Task(subagent_type="rewriter", prompt=<built prompt>)` for in-session triggers, where the `<built prompt>` is the verbatim stdout of `python -m debrief.launcher build_rewrite_prompt --project-root .`. The agent returns the brief markdown as its task output; the consultant pipes the output through `python -m debrief.launcher write_brief` for validation + atomic write.
...
**BC-3.20a `script-writer` agent-card invocation-mode constraint (BUG-AUDIT-98, RETIRED by BUG-AUDIT-102).** *(Pre-fix: the agent card stated that direct Task-tool dispatch was unsupported because the structured-input contract was built only inside the launcher CLI. Post-BUG-AUDIT-102, the structured prompt can be assembled by the new `build_script_prompt` CLI (BC-3.20b) and fed to a Task dispatch — Task is now the canonical path for in-session triggers. The "direct Task dispatch with a free-form prompt" failure mode still exists conceptually — agents need their structured input — but the consultant always satisfies it via the build_script_prompt CLI, so end users never encounter it.)* The card MUST instead document the canonical Task-dispatch flow per BC-5.21 (amended). The pre-fix prose ("`Task`-dispatched invocation with a free-form prompt is unsupported") is RETIRED and SHOULD be replaced with a description of the four-step Task-dispatch chain (build_script_prompt → Task → heredoc draft → write_script). A regression test asserts the new prose is present and the retired prose is absent.

**BC-3.20b `build_script_prompt` subcommand (BUG-AUDIT-102).** `src/unit_3/launcher.py` MUST provide a `build_script_prompt` subcommand wired into `main_new()`'s dispatch table. The subcommand:

- Parses `--project-root` (Path, default `Path.cwd()`).
- Reads project state via existing helpers: brief from `deck_brief.md`, audience from `output/audience.yaml` (parsed via `_parse_audience_yaml`), timeline from `output/timeline.jsonl`, dialog from `.debrief/dialog.jsonl`, slides from `deck_state.json` (filtered to `status == "approved"`), existing `speaker_script.md` (when present, as co-writer baseline).
- Reads `deck_state.presentations` to compute the active presentation folder for downstream context (used by `build_script_writer_inputs`).
- Calls the existing `build_script_writer_inputs(...)` helper with the same 200K-token-cap dialog truncation logic that today's `main_script_writer` uses (the dialog is sized first against the fixed inputs via a placeholder pass, then truncated head-first via `truncate_dialog_to_token_cap`, then re-assembled).
- Writes the assembled prompt to **stdout** (not stderr — the consultant captures via Bash for Task's `prompt` argument). No headers, no JSON wrapping — the raw prompt text.
- Exit codes: 0 on success; 1 only if the deck has zero approved non-backup slides (the precondition that today's `main_script_writer` enforces — there is nothing to script). Read-only — does not modify any file.
- The subcommand MUST appear in the usage-line string printed by the catch-all `else` branch in `main_new()`.

**BC-3.20c `write_script` subcommand (BUG-AUDIT-102).** `src/unit_3/launcher.py` MUST provide a `write_script` subcommand wired into `main_new()`'s dispatch table. The subcommand:

- Parses `--project-root` (Path, default `Path.cwd()`), `--trigger` (one of `"/debrief:script"`, `"deck-complete-finalization"`, default `"/debrief:script"`), and `--draft-path` (Path, default `<project_root>/.debrief/draft/refresh_script.md`).
- Reads the agent's markdown output from `--draft-path`. If the path is missing or empty, logs to `.debrief/script_errors.jsonl` (existing schema, with the trigger value enumerated) and exits 0 (REQ-SCRIPT-WRITER-2 unchanged).
- Re-reads project state needed for the validators: brief, audience YAML (and parsed entries), timeline, dialog, slides_data assembled from `deck_state.slides`. The validators take these as inputs alongside the agent's response_text — they cannot be skipped or simplified.
- Runs the SAME six guardrails in the SAME canonical order as today's `main_script_writer`: (1) `validate_script_structure` (blocker), (2) `validate_script_traceability` (blocker), (3) `validate_script_roster_mentions` (blocker), (4) `validate_script_length_budget` (warning — logged with `error_class: warning_length_overrun`, NOT a blocker), (5) `validate_script_voice_drift` (warning — logged with `error_class: warning_voice_drift`, NOT a blocker, and only when `existing_speaker_script` is non-empty per BC-5.21 co-writer-mode rule), (6) per BC-5.21 the no-new-positions guardrail is prompt-only — no code-side validator. Blocker failures log + exit 0.
- On all blockers passed: invokes `backup_speaker_script(project_root)` per BC-11.20 (backup-before-overwrite) — this is unchanged from `main_script_writer`. Failures here are logged via `log_script_error(error_class="backup_write_failure", …)` but do NOT block the new write per BC-11.20.
- Atomically writes the new `speaker_script.md` via `_atomic_write_text(script_path, response_text)`. Write failure logs and exits 0.
- Emits the `script_done` timeline event via `append_timeline_event(...)` with the same payload schema today's `main_script_writer` emits: `presentation_folder` (from the most recent presentation record or `""`), `slide_count` (main slides only), `backup_slide_count`. Timeline-emit failure is logged but does NOT block.
- On success: removes the draft at `--draft-path` (cleanup). The script-writer has no sentinel analogous to the rewriter's `.brief_stale` — there is no PreCompact-deferred-synthesis flow for the script.
- Always exits 0 — REQ-SCRIPT-WRITER-2 unchanged.
- Read-then-write only on `speaker_script.md`, `.debrief/script_backups/*.md` (via `backup_speaker_script`), `.debrief/script_errors.jsonl`, `output/timeline.jsonl` (via `append_timeline_event`), `.debrief/draft/refresh_script.md` (deletion). NEVER touches dialog archive, slide files, or any file outside this set.
...
- **Invocation pattern (amended by BUG-AUDIT-101).** The rewriter is dispatched via the consultant's `Task` tool for in-session triggers (`/debrief:refresh-brief`, `/debrief:quit` flush, session-start sentinel-detected refresh). The consultant orchestrates: (a) Bash `python -m debrief.launcher build_rewrite_prompt --project-root .` to assemble the structured user message; (b) `Task(subagent_type="rewriter", prompt=<built prompt>)` returning the agent's markdown output (this path uses Claude Code's session credential — OAuth or API key — as opposed to requiring a separate `ANTHROPIC_API_KEY` for the launcher subprocess); (c) Bash `python -m debrief.launcher write_brief --project-root . --trigger <trigger>` to validate + atomically write. PreCompact uses a separate, capture-only path (BC-3.18c) that does NOT call the model — synthesis is deferred to next session start. The pre-BUG-AUDIT-101 hybrid invocation pattern (launcher reads card + calls SDK directly) is RETIRED for synthesis but the launcher continues to handle PreCompact's transcript capture and the `write_brief` validate+atomic-write path. See BUG-AUDIT-101 for rationale.
...
- **Invocation pattern (amended by BUG-AUDIT-102).** The script-writer is dispatched via the consultant's `Task` tool for in-session triggers (`/debrief:script` and the `deck-complete-finalization` cascade in the consultant's 4-step finalization). The consultant orchestrates: (a) Bash `python -m debrief.launcher build_script_prompt --project-root .` assembles the structured user message; (b) `Task(subagent_type="script-writer", prompt=<built prompt>)` returns the agent's markdown output, using Claude Code's session credential — no separate `ANTHROPIC_API_KEY` required; (c) Bash heredoc to stage the agent's output at `.debrief/draft/refresh_script.md`; (d) Bash `python -m debrief.launcher write_script --project-root . --trigger <trigger>` runs the six guardrails, backup-before-overwrite, atomic write, `script_done` event emission, draft cleanup. The `/debrief:handout-cascade` trigger remains on the legacy direct-SDK `main_script_writer` path (cycle 102 transition; cycle 103 decides cleanup) — this requires `ANTHROPIC_API_KEY` and is the only residual auth-key-required path. The pre-BUG-AUDIT-102 hybrid invocation pattern (launcher reads card + calls SDK directly) is RETIRED for in-session triggers but the launcher continues to handle the cascade trigger and the `write_script` validate+atomic-write path. See BUG-AUDIT-102 for rationale.
...
**BC-5.16a Session-start sentinel detection + brief-refresh dispatch (BUG-AUDIT-101).** `agents/consultant.md` MUST contain a dedicated section titled exactly `## Brief Refresh Dispatch` that codifies the four-trigger Task-dispatch protocol for the rewriter. The section MUST:

1. **Name the sentinel.** State that PreCompact's hook does not synthesize the brief — it captures dialog turns and writes `<project_root>/.debrief/.brief_stale` as a deferred-synthesis flag. The sentinel's schema is documented in BC-3.18c.

2. **Document the four triggers.** Enumerate the moments at which the consultant orchestrates a rewriter dispatch:
   - **Session start (sentinel-detected).** After reading `debrief_state.json` and `deck_state.json` and before any user-facing reply, check for `.debrief/.brief_stale`. If present, run the dispatch with `--trigger session_start_recovery`. On success, the `write_brief` CLI removes the sentinel automatically (BC-3.18b). On failure, the sentinel remains; log to `.debrief/rewrite_errors.jsonl` and continue.
   - **Manual `/debrief:refresh-brief`.** User-invoked. Run the dispatch with `--trigger /debrief:refresh-brief`.
   - **`/debrief:quit` flush.** Final consolidation at session end. Run the dispatch with `--trigger /debrief:quit`.
   - **NOT PreCompact.** The PreCompact hook handles itself per BC-3.18c.

3. **Document the four-step dispatch protocol.** For every trigger above, the consultant MUST execute these four steps in order, via Bash + Task:
   1. Bash: `python -m debrief.launcher build_rewrite_prompt --project-root .` — capture the structured prompt from stdout.
   2. `Task(subagent_type="rewriter", prompt=<captured prompt>)` — dispatch via Claude Code's session credential (no separate API key needed). Capture the agent's markdown output.
   3. Bash heredoc: write the agent's markdown verbatim to `.debrief/draft/refresh_brief.md` (the consultant uses `cat > ... <<'EOF' ... EOF` rather than the Write tool to keep the project-scoped write-auth hook semantics simple).
   4. Bash: `python -m debrief.launcher write_brief --project-root . --trigger <trigger>` — validates + atomically writes `deck_brief.md` + `output/audience.yaml`, updates rewrite metadata, removes the draft file, removes the sentinel.

4. **Document the failure-handling.** If any step fails (Task dispatch error, validation rejection in `write_brief`, atomic-write error), the consultant MUST surface a one-line summary to the user (suggested phrasing: *"Brief refresh failed at step <N>: <reason>. The previous deck_brief.md remains in place; run `/debrief:refresh-brief` to retry."*) and continue. The dialog archive captures every turn regardless, so brief staleness is recoverable but not data-losing.

5. **Document the credential model.** State explicitly that this dispatch uses Claude Code's session credential (OAuth or whatever the user is authenticated with) — `ANTHROPIC_API_KEY` is NOT required for any trigger. Reference BUG-AUDIT-101 for the rationale.

The project `CLAUDE.md` template (`templates/project_claude.md`) "On Session Start" section MUST also reference the sentinel check as step 0 (or earliest applicable step), pointing the agent to the consultant card's `## Brief Refresh Dispatch` for the full protocol. The two locations carry the same obligation: the template gates first-turn behavior; the agent card carries the canonical protocol.

A regression test in `tests/regressions/test_bug_audit_101_rewriter_oauth.py` enforces: (a) the `## Brief Refresh Dispatch` section exists in `agents/consultant.md`; (b) all four triggers are named; (c) the four-step protocol is documented with the exact CLI invocations; (d) the credential model statement is present; (e) `templates/project_claude.md` references the sentinel check. See BUG-AUDIT-101 for the rationale.

**BC-5.16b Script-generation dispatch (BUG-AUDIT-102).** `agents/consultant.md` MUST contain a dedicated section titled exactly `## Script Generation Dispatch` parallel in shape to BC-5.16a's `## Brief Refresh Dispatch`. The section MUST:

1. **Name the trigger surface.** Two in-session triggers: (a) direct `/debrief:script` invocation by the user, (b) the `deck-complete-finalization` cascade (step 2 of the consultant's 4-step finalization sequence: `/debrief:refresh-brief` → `/debrief:script` → `/debrief:export` → `/debrief:handout`, per BC-5.21). Plus one residual subprocess-bound trigger: `/debrief:handout-cascade`, which fires inside the handout subprocess when `speaker_script.md` is missing — this trigger remains on the legacy `main_script_writer` direct-SDK path until cycle 103 cleanup decides what to do with it.

2. **Document the four-step Task-dispatch protocol.** Identical shape to BC-5.16a but with script-writer specifics:
   1. Bash: `python -m debrief.launcher build_script_prompt --project-root .` — capture stdout. The CLI assembles the structured user message (deck brief + audience + timeline + truncated dialog + slides + existing speaker_script + external_documents v1-empty placeholder) with the same 200K-token-cap dialog truncation as today's launcher.
   2. `Task(subagent_type="script-writer", prompt=<captured prompt>)` — dispatch via Claude Code's session credential. Capture the agent's markdown output.
   3. Bash heredoc: write the agent's markdown to `.debrief/draft/refresh_script.md`.
   4. Bash: `python -m debrief.launcher write_script --project-root . --trigger <trigger>` — runs the six guardrails (three blockers + two warnings), backup-before-overwrite, atomic write, `script_done` event emission, draft cleanup.

3. **Document the credential model.** State explicitly that `/debrief:script` and `deck-complete-finalization` use Claude Code's session credential (OAuth or API key) — `ANTHROPIC_API_KEY` is NOT required for either of these triggers. Reference BUG-AUDIT-102 for the rationale. State that `/debrief:handout-cascade` remains on the legacy path requiring `ANTHROPIC_API_KEY`; users who hit this path see the BUG-AUDIT-98 stderr line and should run `/debrief:script` manually as a workaround until cycle 103 cleanup retires or restructures the cascade.

4. **Document the failure-handling.** If any step fails (Task dispatch error, blocker validation rejection in `write_script`, atomic-write error), the consultant MUST surface a one-line summary to the user (suggested phrasing: *"Script generation failed at step <N>: <reason>. The previous `speaker_script.md` remains in place; run `/debrief:script` to retry."*). Warnings (length-budget, voice-drift) are logged but the script IS still written; the consultant SHOULD surface them to the user as advisory information without blocking.

5. **Reference the 4-step finalization sequence.** State that the consultant's deck-complete-finalization cascade (BC-5.21 §"hybrid invocation pattern", amended) executes step 2 of the sequence by running THIS dispatch with `--trigger deck-complete-finalization`. Steps 1, 3, 4 (refresh-brief, export, handout) are unchanged.

A regression test in `tests/regressions/test_bug_audit_102_script_writer_oauth.py` enforces: (a) the `## Script Generation Dispatch` section exists; (b) both in-session triggers are named, plus the cascade trigger is named as a transitional residual; (c) the four-step protocol is documented with the exact CLI invocations; (d) the credential model statement is present (Task uses session credential; cascade still needs API key); (e) `commands/script.md` describes the new flow.

**BC-5.16c Handout-generation dispatch with script-precondition self-heal (BUG-AUDIT-103).** `agents/consultant.md` MUST contain a dedicated section titled exactly `## Handout Generation Dispatch` parallel in shape to BC-5.16a's `## Brief Refresh Dispatch` and BC-5.16b's `## Script Generation Dispatch`. The section MUST:

1. **State the precondition.** `/debrief:handout` requires `<project_root>/speaker_script.md` to exist. The handout subprocess (BC-11.16) no longer auto-cascades to the script-writer when the script is missing — that auto-cascade was retired in BUG-AUDIT-103 because it ran the SDK in a subprocess that had no Claude Code session to dispatch into. The convenience of "run handout standalone and get a script auto-generated" is preserved, but the orchestration moves UP to the consultant.

2. **Document the precondition self-heal.** When the user invokes `/debrief:handout`, the consultant MUST FIRST check for `speaker_script.md` in the project root. If absent, the consultant runs the BUG-AUDIT-102 four-step Task-dispatch chain to generate the script (per BC-5.16b's `## Script Generation Dispatch` protocol with `--trigger /debrief:script`), then proceeds with the handout. If present, the consultant proceeds with the handout directly. The check is a single Bash `[[ -f speaker_script.md ]]` test — fast, no model call.

3. **Document the dispatch protocol.** After the precondition is satisfied (script exists), the consultant invokes the handout subprocess via Bash:

   ```bash
   python -m debrief.utility_skills handout --mode <2up|4up> --project-root . [--include-backup]
   ```

   This is the existing handout invocation; BUG-AUDIT-103 does not change it. The handout subprocess now has a hard precondition (script exists) — if the precondition is violated despite the consultant's check (e.g., file deleted between check and invoke, or direct subprocess call from outside the consultant), `main_handout` exits 2 with `"speaker_script.md is missing. Run /debrief:script first, then retry /debrief:handout."`.

4. **Document the credential model.** State that this dispatch flow uses Claude Code's session credential — `ANTHROPIC_API_KEY` is NOT required for the handout flow OR for the script-precondition self-heal. The retired auto-cascade required the API key; cycle 103's lift-to-consultant eliminates that requirement.

5. **Document the failure-handling.** If the precondition self-heal fails (script generation rejected by validators, Task dispatch error, etc.), the consultant MUST surface a one-line summary to the user (suggested phrasing: *"Could not auto-generate `speaker_script.md` for the handout: <reason>. Run `/debrief:script` to retry, then re-run `/debrief:handout`."*) and abort the handout dispatch. The handout subprocess is NOT invoked when the precondition cannot be satisfied — preventing the "every cell shows '(no notes available)'" failure mode (BUG-AUDIT-100) at the orchestration layer rather than at the parser layer.

A regression test in `tests/regressions/test_bug_audit_103_anthropic_optional.py` enforces: (a) the `## Handout Generation Dispatch` section exists; (b) the precondition self-heal step is documented; (c) `commands/handout.md` describes the new flow; (d) `main_handout` no longer imports or calls `main_script_writer` (the auto-cascade is gone); (e) `main_handout`'s missing-script branch exits 2 with a helpful message rather than 0 with a degraded run.
...
**BC-11.16 handout fail-fast preconditions (BUG-AUDIT-21 — amended by BUG-AUDIT-84, further amended by BUG-AUDIT-103).** `main_handout` validates preconditions at entry, BEFORE any Playwright import or browser launch, in this explicit order: (1) `deck_state.json` exists and is readable; (2) at least one slide has `status == "approved"` AND `backup != True`; (3) `<project_root>/speaker_script.md` exists; (4) `importlib.util.find_spec("playwright")` returns a non-`None` spec (see BC-11.7). On any failure, the function prints a descriptive message to stderr identifying the specific missing prerequisite (the message MUST name the prerequisite and MUST NOT be a raw Python exception traceback) and exits with code 2. The playwright check is deliberately attempted last so that missing-project, missing-slides, or missing-script runs produce accurate error messages. **(BUG-AUDIT-103 amendment:** the BUG-AUDIT-84 auto-cascade clause — *"absence triggers consultant auto-cascade to run `/debrief:script` first rather than a hard failure"* — is RETIRED. `main_handout` no longer imports or invokes `main_script_writer` from inside its subprocess; the script-precondition self-heal lives at the consultant orchestration layer per BC-5.16c. When `speaker_script.md` is absent, `main_handout` exits 2 with the message `"speaker_script.md is missing. Run /debrief:script first, then retry /debrief:handout."`. The orchestration layer's check happens BEFORE the subprocess invocation, so this hard-failure path is normally unreachable in canonical user flows; it remains as defensive output for direct subprocess calls or check-and-invoke races. **The exit-code change from 0 to 2** reflects this — the missing-script case is now an unexpected state, not a normal-flow occurrence absorbed silently.) See REQ-HAND-6, BUG-AUDIT-21, BUG-AUDIT-84, and BUG-AUDIT-103.
