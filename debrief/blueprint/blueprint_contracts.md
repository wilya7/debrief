...
**BC-5.11 paper_analyzer invocation.** When the user provides a paper PDF path during discovery, the Consultant invokes `python -m debrief.paper_analyzer --pdf <path> --paper-slug <slug> --project-root <path>` via Bash. The Consultant does not import `fitz` directly.
...
**BC-11.6 script folder selection.** `main_script_generator` uses the most recent presentation folder (last entry in `presentations` array). NNN for the script version is `script_count + 1` at the time of generation, zero-padded to 3 digits (e.g., `script_v001.md`, `script_v002.md`). *(BUG-AUDIT-21: the handout versioning rule that was formerly bundled into this BC has been moved to BC-11.17 and no longer routes through the presentation folder. BC-11.6 now covers script versioning only; the handout path is decoupled.)*
...
**BC-11.15a handout notes source precedence (BUG-AUDIT-68 / REQ-HAND-NOTES-1, REQ-HAND-NOTES-2).** `generate_layout_html` MUST resolve each slide's notes text via a deterministic three-level precedence, applied independently per slide:
...
**BC-11.16 handout fail-fast preconditions (BUG-AUDIT-21).** `main_handout` validates preconditions at entry, BEFORE any Playwright import or browser launch, in this explicit order: (1) `deck_state.json` exists and is readable; (2) at least one slide has `status == "approved"` AND `backup != True`; (3) `importlib.util.find_spec("playwright")` returns a non-`None` spec (see BC-11.7). On any failure, the function prints a descriptive message to stderr identifying the specific missing prerequisite (the message MUST name the prerequisite and MUST NOT be a raw Python exception traceback) and exits with code 2. The playwright check is deliberately attempted last so that missing-project or missing-slides runs produce accurate error messages. See REQ-HAND-6 and BUG-AUDIT-21.
...
**BC-11.19 Generator output-path contract (BUG-AUDIT-77 / REQ-GEN-PATHS-1).** Every generator command shipped by the plugin MUST write its output ONLY to the path(s) enumerated in the spec's Generator Output Paths table (appended to Section 14 under BUG-AUDIT-77). Current entries:
...
**BC-12.7 Whitespace crop fallback.** If `crop_whitespace` finds that the entire figure image is white (no content pixels), it must return the original pixmap unchanged rather than raising an exception or returning an empty image.
