...
**BC-3.20 `script_writer` subcommand (BUG-AUDIT-84 / REQ-SCRIPT-WRITER-1..4).** `src/unit_3/launcher.py` MUST provide a `script_writer` subcommand wired into `main_new()`'s dispatch table. Mirrors BC-3.18's `rewrite_brief` shape:
...
2. **`## Paper Discussion`** — the open Socratic engagement layer. The section header MUST exist literally. The body describes branching by `paper_role` (Cycle 5 / BUG-AUDIT-90 introduces the taxonomy, `primary_dissection`, `primary_thematic`, `primary_document`, `concept_source`, `background_reference`, `none`) and what discussion shape applies per role. The body's prose is NOT pinned by regression — substantive engagement with the user's content is where LLM judgment is the value, not the smell. The regression test asserts the section header exists and the six `paper_role` values are mentioned, but does NOT pin discussion prose word-for-word.

A single regression test in `tests/regressions/test_bug_audit_89_consultant_paper_sections.py` enforces: (a) both section headers exist in the specified order; (b) the deterministic substrings (command template fragment, all three sub_phase strings, both event names) appear in `## Paper Analyzer Invocation`; (c) all six `paper_role` values appear in `## Paper Discussion` ; (d) the literal phrase "discovery/figure_selection" appears in both sections (transition into the gate from the discussion).
